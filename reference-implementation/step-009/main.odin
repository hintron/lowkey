...
execute :: proc(tokens: [dynamic]Token, source_text: string, allocator: runtime.Allocator) -> map[string]int {
	...
	for token in tokens {
		...
		if dest_identifier.type != .Nil && is_doing_assignment && source_token.type != .Nil {
			dest_var_name := source_text[dest_identifier.start_byte:][:dest_identifier.length]
			source_value : int
			ok : bool
			switch source_token.type {
			case .IdentifierVariable:
				source_var_name := source_text[source_token.start_byte:][:source_token.length]
				if source_value, ok = var_values[source_var_name]; !ok {
			...
			var_values[dest_var_name] = source_value
...
test_execute_009 :: proc(t: ^testing.T) {
	...
	var_values := execute(tokens, source_text, context.temp_allocator)
